...
class CloudSnarf(base.Program):
    """Identify IPv4 hosts behind Cloudflare using certificate data.
       ...
       This program aims on uncovering Cloudflare-protected domains
       ...
       the base servers that are hidden behind Cloudflare."""
    ...
    def run(self):
        try:
            ...
            certificates = [{key.rsplit(".", 1)[-1]: value for key, value in cert.items()} for cert in self.censys.certificates.search(f"{self.arguments.domain} and tags: trusted", fields = fields) if cert]
            ...
            console.print(f"[!] {type(e).__name__}{':' if str(e) else ''}", color = "red")
            if str(e):
